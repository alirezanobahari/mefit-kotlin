...
/**
 * run [body] if [R.bool.is_tablet] is true
 */
fun Context.onTablet(body: () -> Unit) {
    if (booleans[R.bool.is_tablet])
        body()
}

/**
 * run [body] if [R.bool.is_tablet] is false
 */
fun Context.onMobile(body: () -> Unit) {
    if (booleans[R.bool.is_tablet].not())
        body()
}

/**
 * run [body] if screen orientation is portrait
 */
fun Context.onPortrait(body: () -> Unit) {
    if (resources.configuration.orientation == Configuration.ORIENTATION_PORTRAIT)
        body()
}

/**
 * run [body] if screen orientation is landscape
 */
fun Context.onLandScape(body: () -> Unit) {
    if (resources.configuration.orientation == Configuration.ORIENTATION_LANDSCAPE)
        body()
}

fun Context.isActivityInTaskStack(activityClass: KClass<*>): Boolean {
    val manager = getSystemService(ACTIVITY_SERVICE) as ActivityManager
    val taskList = manager.getRunningTasks(50)
    return taskList.count { it.topActivity.className == activityClass.jvmName } > 0
}
